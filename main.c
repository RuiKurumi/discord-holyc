...
void emitFile(Cctrl *cc, AoStr *asmbuf, CliArgs *args) {
    ...
    if (args->emit_object) {
        ...
        if (args->run) {
            ...
            aoStrCatPrintf(run_cmd,"%s -L%s/lib %s %s %s %s "CLIBS" -ltos -lm -lssl -lcrypto && ./a.out && rm ./a.out %s",
                    cc->CC,
                    args->install_dir,
                    ASM_TMP_FILE,
                    ofiles->data,
...
void assemble(Cctrl *cc, CliArgs *args) {
    ...
    if (args->run) {
        ...
        aoStrCatPrintf(run_cmd,
                "%s -L"INSTALL_PREFIX"/lib %s "CLIBS" -ltos -lm -lssl -lcrypto && ./a.out && rm ./a.out",
                cc->CC,
                ASM_TMP_FILE);
        free(buffer);
        int ret = system(run_cmd->data);
